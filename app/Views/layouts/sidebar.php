    ...
    <div class="sidebar-content js-simplebar">
        ...
        <ul class="sidebar-nav">
            ...
            foreach ($MenuCategory as $mCategory) :
                ...
                if (in_array($mCategory['menu_category'], $processedCategories)) {
                    continue;
                }
                $processedCategories[] = $mCategory['menu_category'];
            ?>
                ...
                foreach ($Menu as $menu) :
                    // Skip if menu already processed
                        ...
                        <li class="sidebar-item <?= ($segment == $menu['url']) ? 'active' : ''; ?>">
                            ...
                            <ul id="<?= $menu['url'] ?>" class="sidebar-dropdown list-unstyled collapse <?= ($segment == $menu['url']) ? ' show' : ''; ?> " data-bs-parent="#sidebar">
                                <?php foreach ($SubMenu as $subMenu) : 
                                    // Skip if submenu already processed
                                    if (in_array($subMenu['menu_id'], $processedSubMenus)) {
                                        continue;
                                    }
                                    $processedSubMenus[] = $subMenu['menu_id'];
                                ?>
